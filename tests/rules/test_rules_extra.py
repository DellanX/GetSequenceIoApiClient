...
async def test_async_list_rules_and_get_rule():
    ...
    r = await client.rules.async_get_rule("r1")
...
async def test_async_trigger_rule_returns_execution_id():
    ...
    client = SequenceApiClient(session, "token")
    exec_id = await client.rules.async_trigger_rule("r1", execute_amount=50, simulation=True, idempotency_key="k")
...
async def test_async_list_rules_filters_params():
    ...
    client = SequenceApiClient(session, "token")
    await client.rules.async_list_rules(source_id="s-1", page=1, page_size=5)
